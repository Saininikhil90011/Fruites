...
const { Contact, Product, User, Data } = require("./conn");
...
app.post("/add_data", async (req, res) => {
  try {
    const obj = req.body;
    const newUser = await Data.create(obj);
    const saveUser = await newUser.save();
    return res.json({ data: "", message: "Data inserted successfully" });
  } catch (err) {
...
app.get("/get_data", async (req, res) => {
  try {
    ...
    return res.json({ data: data, message: "Data inserted successfully" });
  } catch (err) {
    return res.json({ data: "", message: err.message });
  }
});
